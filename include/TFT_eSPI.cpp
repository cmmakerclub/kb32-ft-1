...
{
  CS_H;
  _pin_delay; // Just in case it has been left low
...
void TFT_eSPI::writedata(uint8_t d)
{
  ...
  DC_D;
  _pin_delay; // Play safe, but should already be in data mode

  tft_Write_8(d);

  CS_L;
  _pin_delay; // Allow more hold time for low VDI rail
...
uint8_t TFT_eSPI::readcommand8(uint8_t cmd_function, uint8_t index)
{
  ...
  CS_H;
  _pin_delay; // Some displays seem to need CS to be pulsed here, or is just a delay needed?
  CS_L;
  _pin_delay;
...
void TFT_eSPI::pushColor(uint16_t color, uint32_t len)
{
  ...
  if (len)
    spi.writePattern(&colorBin[0], 2, 1);
  len--;
  while (len--)
  {
    WR_L;
...
void TFT_eSPI::drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color)
{
  ...
  else // x increments every iteration (x0 is x-axis, and y0 is y-axis)
  {
    ...
    for (; x0 <= x1; x0++)
    {
      if ((x0 >= 0) && (y0 >= 0) && (y0 < (int32_t)_height))
        break;
      err -= dy;
      if (err < 0)
      {
...
int16_t TFT_eSPI::drawString(const char *string, int32_t poX, int32_t poY, uint8_t font)
{
  ...
  if (printUTF8)
  {
    return drawUTF8String(string, poX, poY, font);
  }
  int16_t sumX = 0;
...
int16_t TFT_eSPI::drawUTF8String(const char *s, int poX, int poY, int font)
{
  int txBuffer[256];
  int ib = 0;
  ...
  while ((c = s[i]) != 0)
  {
    ...
    else if (wc >= 0xE48 && wc <= 0xE4c)
    {
      if (lc == 0xE1B || lc == 0xE1D || lc == 0xE1F)
      {
        wc = wc + (0xE6B - 0xE48);
      }
      else if (lc >= 0xE34 && lc <= 0xE37)
      {
        wc = wc + (0xE7B - 0xE48);
      }
      else if (lc >= 0xE65 && lc <= 0xE69)
      {
        wc = wc + (0xE75 - 0xE48);
      }
    }

    //Serial.printf( "Char %i: %X\n", ib, wc );
    txBuffer[ib++] = wc;
    lc = wc;

    if (wc >= 0xE01 && wc <= 0xE7F)
    {
      gfxFont = gfxUTF8Font[1];
    }
    else
    {
      gfxFont = gfxUTF8Font[0];
    }

    if (wc > pgm_read_word(&gfxFont->last))
      wc = pgm_read_word(&gfxFont->first);
    ;
    wc -= pgm_read_word(&gfxFont->first);
    GFXglyph *glyph = &(((GFXglyph *)pgm_read_dword(&gfxFont->glyph))[wc]);

    int xa = pgm_read_byte(&glyph->xAdvance);
    sumX += xa;
  }
  int assending = glyph_ab * textsize;
  int descending = +glyph_bb * textsize;
  int cheight = assending + descending;
  int x = 0;
  int y = assending;
  uint8_t padding = 1, baseline = 0;
  switch (textdatum)
  ...
  fillRect(poX + x, poY + y - assending, sumX, cheight, textbgcolor);
  int16_t padXc = poX + x + sumX; // Maximum left side padding
  int16_t padXo = poX + x;
  ...
  for (int i = 0; i < ib; i++)
  {
    ...
    x += drawChar(wc, poX + x, poY + y, font);
  }
  switch (padding)
  {
  case 1:
  ...
  case 2:
    ...
    //if (padXc > poX+x)
    //  padXc = poX + x;
    fillRect(padXo - padXc, poY, (padX - sumX) >> 1, cheight, textbgcolor);
    break;
  case 3:
    padXc = (padX - sumX);
    //if (padXc > padX)
    //  padXc = padX;
    fillRect(padXo - padXc, poY, (padX - sumX), cheight, textbgcolor);
    break;
  }
  return sumX;
}

int16_t TFT_eSPI::textUTF8Width(const char *s, int font)
{
  int i = 0;
  char c;
  int sumX = 0;
...
void TFT_eSPI::setUTF8Font(const GFXfont *f0, const GFXfont *f1, const GFXfont *f2)
{
  setFreeFont(f0);

  gfxUTF8Font[0] = (GFXfont *)f0;
  gfxUTF8Font[1] = (GFXfont *)f1;
  gfxUTF8Font[2] = (GFXfont *)f2;

  glyph_ab = 0;
  glyph_bb = 0;
  for (int fx = 0; fx < 3; fx++)
  {
    if (gfxUTF8Font[fx] == NULL)
      continue;
    uint16_t numChars = pgm_read_word(&gfxUTF8Font[fx]->last) - pgm_read_word(&gfxUTF8Font[fx]->first);

    //Serial.printf( "[DEBUG] font %i: %i\n", fx, numChars );
    // Find the biggest above and below baseline offsets
    for (uint8_t c = 0; c < numChars; c++)
    {
      GFXglyph *glyph1 = &(((GFXglyph *)pgm_read_dword(&gfxUTF8Font[fx]->glyph))[c]);
